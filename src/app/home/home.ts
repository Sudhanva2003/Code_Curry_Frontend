...
fetchRestaurants() {
  ...
  this.api.get(`Filter/Restaurants?sort=${this.restaurantSort}`).subscribe({
    next: (data: any) => {
      ...
      // Wait for distances to be populated, then sort if needed
      this.getDistanceForRestaurants().then(() => {
        if (this.restaurantSort === 'distance') {
...
getDistanceForRestaurants(): Promise<void> {
  return new Promise((resolve) => {
    ...
    this.api.get(`customer/ViewUser/${user.userId}`).subscribe({
      next: (userData: any) => {
        ...
        // Build array of promises for all restaurants
        const promises = this.restaurants.map((restaurant) => {
          return new Promise<void>((res) => {
            // If restaurant has no address, mark N/A and resolve
            ...
            this.api.get(`customer/Distance?restAddress=${encodeURIComponent(restaurant.address)}&customerAddress=${encodeURIComponent(customerAddress)}`).subscribe({
              next: (distance: number) => {
                // assign numeric distance (ensure it's a number)
                restaurant.distance = typeof distance === 'number' ? distance : Number(distance);
                res();
              },
              error: (error) => {
        ...
        // Wait for all distance calls to finish
  ...
  if (this.filterType === 'none') {
    ...
    result = this.menu.filter((f) => {
      const category = f.category?.toLowerCase() || '';
      if (filter === 'veg') {
        return category === 'veg' || category === 'vegan';
      }
